  ...
  useEffect(() => {
    return async () => {
      try {
        ...
        if(req.data.message==='PREMIUM MEMBER'){
          setpremiumbtn(true)
        }
  ...
  const premiumHandler = async () => {
    ...
    try {
      ...
      rzp.on('payment.failed',async function(res){
        try{
          const req=await axios.get('http://localhost:9000/transactionfailed',{headers: { Authorization: localStorage.getItem("token") }})
          console.log(req)
          rzp.close()
        }
        catch(err){
